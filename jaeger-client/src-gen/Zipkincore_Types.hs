...
from_Endpoint record = T.TStruct $ Map.fromList $ M.catMaybes
  ...
  , (\_v2 -> (4, ("ipv6",T.TString _v2))) <$> endpoint_ipv6 record
  ]
...
to_Endpoint (T.TStruct fields) = Endpoint{
  ...
  endpoint_ipv6 = P.maybe (P.Nothing) (\(_,_val4) -> P.Just (case _val4 of {T.TString _val8 -> _val8; _ -> P.error "wrong type"})) (Map.lookup (4) fields)
  }
...
typemap_Endpoint = Map.fromList [(1,("ipv4",T.T_I32)),(2,("port",T.T_I16)),(3,("service_name",T.T_STRING)),(4,("ipv6",T.T_STRING))]
...
from_BinaryAnnotation record = T.TStruct $ Map.fromList $ M.catMaybes
  ...
  , (\_v19 -> P.Just (2, ("value",T.TString _v19))) $ binaryAnnotation_value record
...
write_BinaryAnnotation :: (T.Protocol p, T.Transport t) => p t -> BinaryAnnotation -> P.IO ()
write_BinaryAnnotation oprot record = T.writeVal oprot $ from_BinaryAnnotation record
encode_BinaryAnnotation :: (T.Protocol p, T.Transport t) => p t -> BinaryAnnotation -> LBS.ByteString
...
to_BinaryAnnotation (T.TStruct fields) = BinaryAnnotation{
  ...
  binaryAnnotation_value = P.maybe (binaryAnnotation_value default_BinaryAnnotation) (\(_,_val21) -> (case _val21 of {T.TString _val23 -> _val23; _ -> P.error "wrong type"})) (Map.lookup (2) fields),
...
read_BinaryAnnotation :: (T.Transport t, T.Protocol p) => p t -> P.IO BinaryAnnotation
read_BinaryAnnotation iprot = to_BinaryAnnotation <$> T.readVal iprot (T.T_STRUCT typemap_BinaryAnnotation)
decode_BinaryAnnotation :: (T.Protocol p, T.Transport t) => p t -> LBS.ByteString -> BinaryAnnotation
...
typemap_BinaryAnnotation = Map.fromList [(1,("key",T.T_STRING)),(2,("value",T.T_STRING)),(3,("annotation_type",T.T_I32)),(4,("host",(T.T_STRUCT typemap_Endpoint)))]
